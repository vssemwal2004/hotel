...
export default function WorkerCheckInPage() {
  ...
  const filterBookings = () => {
    ...
    // AND currently ongoing stays (not checked out yet). Exclude cancelled/completed.
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-2">
            <LogInIcon className="text-blue-600" size={24} />
            Currently Checked-In Guests
          </h1>
          <p className="text-xs md:text-sm text-gray-600 mt-0.5">Guests currently staying at the hotel</p>
        </div>
        <button
      ...
      <div className="mb-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white rounded-lg p-2.5 border-l-4 border-blue-500 shadow-sm">
            <p className="text-xs text-gray-600 mb-0.5">Current Guests</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full pl-9 pr-8 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none bg-white"
            >
              <option value="all">All Current Guests</option>
              <option value="today">Checked In Today</option>
              <option value="week">Checked In This Week</option>
              <option value="month">Checked In This Month</option>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-3">
          <h2 className="text-base md:text-lg font-bold flex items-center gap-2">
            <LogInIcon size={20} />
            Currently Staying Guests ({filteredBookings.length})
          </h2>
        </div>

        {filteredBookings.length === 0 ? (
          <div className="text-center py-12">
            <LogInIcon size={48} className="mx-auto text-gray-300 mb-3" />
            <p className="text-gray-600 text-base font-semibold mb-1">No current guests</p>
            <p className="text-gray-500 text-sm">
              {searchQuery || dateFilter !== 'all' ? 'Try adjusting your filters' : 'No guests currently checked in'}
            </p>
          </div>
        ) : (
          <>
                    ...
                    <tr key={booking._id} className="hover:bg-blue-50 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        ...
                        <p className="text-xs text-gray-500">
                          {new Date(booking.checkIn).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </td>
                      <td className="px-3 py-2.5">
            ...
            <div className="lg:hidden p-3 space-y-3">
              {filteredBookings.map((booking) => (
                <div key={booking._id} className="bg-gradient-to-br from-white to-blue-50 rounded-lg border-2 border-blue-100 p-3">
